...
const execute: OperationExecuteExport = async (request) => {
  ...
  // Gather facts
  const type = values.type.data as 'ecc' | 'rsa'
  const userId = {}
  const passphrase = values.passphrase.data as string
  const armored = values.armored.data as boolean

  // Generate key pair
  const { privateKey, publicKey } = await openpgp.generateKey({
    type,
    curve: values.eccCurve.data as openpgp.KeyOptions['curve'],
    rsaBits: values.rsaBits.data as number,
    userIDs: [userId],
    passphrase,
    format: 'object'
  })
